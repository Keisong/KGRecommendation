...
class KGCN(object):
    ...
    def get_neighbors(self, seeds):
        ...
        entities = [seeds]
        relations = []
        for i in range(self.n_iter):   # n_iter means n-layer neighbors, maximum neighbor_sample_size neighbors per layer
            temp1=entities[i]
            temp2=self.adj_entity
            temp3=tf.gather(self.adj_entity, entities[i]), [self.batch_size, -1]
            neighbor_entities = tf.reshape(tf.gather(self.adj_entity, entities[i]), [self.batch_size, -1]) # gather: extract items ->(items , item_index )
