...
class RippleNet(object):
    ...
    def _build_model(self):
        ...
        o_list = self._key_addressing() # obtain Vector represented by green rectangle

        self.scores = tf.squeeze(self.predict(self.item_embeddings, o_list))
        # squeeze() tries to delete all dimension equals to 1, reverse to expand_dims()
        # predict() multiplies user embedding (prob) and updated item embedding to get final prob
    ...
    def _key_addressing(self):
        ...
        for hop in range(self.n_hop):
            ...
            h_expanded = tf.expand_dims(self.h_emb_list[hop], axis=3) # expand a 1 dimension at position where axis=3
    ...
    def _build_loss(self):
        ...
        # cross_entropy for RS

        self.kge_loss = 0
        for hop in range(self.n_hop):
            h_expanded = tf.expand_dims(self.h_emb_list[hop], axis=2) # ? 32 1 16
            t_expanded = tf.expand_dims(self.t_emb_list[hop], axis=3) # ? 32 16 1
            # self.r_emb_list[hop] # ? 32 16 16
            # tf.matmul(h_expanded, self.r_emb_list[hop]) # ? 32 1 16
            # tf.matmul(tf.matmul(h_expanded, self.r_emb_list[hop]), t_expanded) # ? 32 1 1
            hRt = tf.squeeze(tf.matmul(tf.matmul(h_expanded, self.r_emb_list[hop]), t_expanded)) # ? 32
